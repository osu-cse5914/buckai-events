...
vi.mock("@tanstack/react-router", () => ({
  ...
  Link: ({
    ...
    if (params?.collectionId && to === "/you/collections/$collectionId") {
      href = `/you/collections/${params.collectionId}`;
    }
    if (search) {
...
describe("[phase:1] [regression:always] EventDetailPage", () => {
  ...
  it("TC-EVT-048: uses the collection as the back destination when arriving from a collection", async () => {
    mockRouteSearch = {
    ...
    expect(await screen.findByRole("link", { name: "Back to Music" })).toHaveAttribute(
      "href",
      "/you/collections/col_1",
    );
  });
