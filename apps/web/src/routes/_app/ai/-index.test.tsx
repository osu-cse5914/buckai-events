...
const state = vi.hoisted(() => {
  ...
  const conversationsDelete = vi.fn();
      ...
      api: {
        v1: {
          conversations: {
            ...
            [":id"]: {
              $delete: (...args: unknown[]) => conversationsDelete(...args),
              messages: {
...
describe("[phase:5] [regression:always] AI Route", () => {
  ...
  it("TC-CONV-011: deletes a conversation thread from the sidebar", async () => {
    ...
    state.conversationsGet
      .mockImplementationOnce(() =>
        Promise.resolve(
          jsonResponse(
            paginated([
              makeConversation({ id: "conv_2", title: "Most recent" }),
              makeConversation({ id: "conv_1", title: "Older chat" }),
            ]),
          ),
        ),
    ...
    state.conversationsDelete.mockResolvedValue(new Response(null, { status: 204 }));

    await renderAiRoute({ conversationId: "conv_1" });

    const olderChatButton = await screen.findByRole("button", { name: /Older chat/ });

    await user.click(
      within(olderChatButton.parentElement as HTMLElement).getByRole("button", {
    ...
    await waitFor(() => {
      expect(state.conversationsDelete).toHaveBeenCalledWith({
        param: { id: "conv_1" },
      });
    });
    await waitFor(() => {
