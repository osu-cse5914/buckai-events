...
import { buildEventWithCreatorAndInteractions, buildInteraction, buildUser } from "./factories";
...
function makeEvent(overrides: Record<string, unknown> = {}, interactionCount = 0) {
...
describe("[phase:4] [regression:always] GET /api/v1/recommendations", () => {
  ...
  it("TC-REC-MODEL-001: boosts events matching the user's interests", async () => {
    ...
    expect(body.items.map((item) => item.id)).toEqual(["evt_music", "evt_sports"]);
  ...
  it("TC-REC-MODEL-002: ranks more popular items ahead when interest match is tied", async () => {
    ...
    expect(body.items.map((item) => item.id)).toEqual(["evt_popular", "evt_less_popular"]);
  });
  ...
  it("TC-REC-MODEL-003: ranks nearer-future items ahead when interest match and popularity are tied", async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { items: Array<{ id: string }> };
    expect(body.items.map((item) => item.id)).toEqual(["evt_tomorrow", "evt_next_month"]);
  });
  ...
  it("TC-REC-MODEL-007: interest-matched event outranks a significantly more popular but unmatched event", async () => {
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { items: Array<{ id: string }> };
    expect(body.items.map((item) => item.id)).toEqual(["evt_matched", "evt_very_popular"]);
  });
  ...
  it("TC-FEED-002: supports EVENT-only filtering", async () => {
    ...
    const res = await app.request(makeAuthRequest("/api/v1/recommendations?type=EVENT"));
  ...
  it("TC-FEED-003: supports GIG-only filtering", async () => {
    ...
    const res = await app.request(makeAuthRequest("/api/v1/recommendations?type=GIG"));
  ...
  it("TC-FEED-004: paginates the ranked result set", async () => {
    ...
    const res = await app.request(makeAuthRequest("/api/v1/recommendations?limit=1&offset=1"));
  ...
  it("TC-FEED-016: applies keyword search before pagination", async () => {
    ...
    const res = await app.request(makeAuthRequest("/api/v1/recommendations?search=career&limit=1"));
...
describe("[phase:6] [regression:always] Recommendation section endpoints", () => {
  ...
  it("TC-FEED-010: popular and upcoming exclude dismissed and ineligible items", async () => {
    ...
    const popularRes = await app.request(makeAuthRequest("/api/v1/recommendations/popular"));
    const upcomingRes = await app.request(makeAuthRequest("/api/v1/recommendations/upcoming"));
